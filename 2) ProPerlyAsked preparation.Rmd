...
fullTimed_df <- left_join(fullTime_df, textGridSyll_df, by = c("file", "t"))
...
```{r F0_constant}
F0_Constant_df <- read.csv("fromConstantijn/data_long.csv")
...
F0_Constant_df <- droplevels(subset(F0_Constant_df, select = -c(start, end, step, stepnumber, f0, jumpkilleffect)))

colnames(F0_Constant_df) <- c("file","interval_label","t","f0_CC")

#####

fullTimeF0_df <- left_join(fullTime_df, F0_Constant_df, by = c("file", "t"))

fullTimeF0_df <- mutate(
  group_by(fullTimeF0_df, file),
  ##  f0_smooth_stretch ('f0_smootch')
  f0_CCsmootch = ifelse(
    (is.na(f0_CC) & t<min(which(!is.na(f0_CC)))),
    f0_CC[min(which(!is.na(f0_CC)))], ifelse(
      (is.na(f0_CC) & t>=max(which(!is.na(f0_CC)))),
      f0_CC[max(which(!is.na(f0_CC)))], f0_CC)),
    f0_CCsmootchInterp = round(na.approx(f0_CCsmootch))
  )

fullTimeF0_df <- droplevels(subset(fullTimeF0_df, select = -f0_CCsmootch))

fixTimedF0_df <- left_join(fixTimed_df, fullTimeF0_df, by = c("file", "t"))

#####

# fixTimedF0_df <- left_join(fixTimed_df, F0_Constant_df, by = c("file", "t"))


# fixTimedF0_df <- mutate(
#   group_by(fixTimedF0_df, file),
#   f0_CCinterp = round(na.spline(f0_CC))#,
#   # f0_CCinterp = ifelse(f0_CCinterp < min(f0_CC, na.rm=T), NA, f0_CCinterp)
#   )
...
bigTable <- read_xlsx("data_tables/Edited_harvest2.xlsx")
...
# > levels(as.factor(smallTable$sex))
# [1] "f" "m"
# > levels(as.factor(smallTable$age))
# [1] "10" "20" "30" "40" "50" "60" "70" "80"
# > levels(as.factor(smallTable$speaker))
...
# > levels(as.factor(smallTable$role))
...
raw_df <- mutate(
  ...
  syll_label = ifelse(Auxiliary=="could", "could", ifelse(Auxiliary=="may", "may", syll_label))
  )
