...
## compute and smooth 1st and 2nd derivatives of the periodic energy curve
    smogPP_1stDer = ifelse(t==0, 0, diff(smogPP_12Hz)*1000), # ifelse(t==0, 0, (smogPP_20Hz - lag(smogPP_20Hz,1))*1000),
...
expSyllNum <- ifelse(useManual==TRUE,
                     ...
                     (length(which(single_token$smogPP_12Hz > 0)) / averageSyll) + 1)
#
## == ## == ## == ## == ## == ## == ## == ## == ## == ## == ##
## == ## == ## == ## == ## == ## == ## == ## == ## == ## == ##
#
...
comp_df <- mutate(
  group_by(comp_df, file),
  auto_bounds = ifelse(t==auto_bounds, auto_bounds, NA)
  )
...
# singleTest <- dplyr::filter(droplevels(comp_df), file=="cut-WPVI_20170802_220000_Action_News_6PM")
...
##  f0_smooth_stretch_interp_smooth_drop interpolation
  f0Floor4real = ifelse(
  smogPP_12Hz < 0.1, NA, f0Post_dropInterp)
...
CoG <- function(df) {
  ...
  f0Floor <- ifelse(df$f0_token_range > 99, 
  # f0Floor <- ifelse(df$f0_speaker_range_median > 99, 
                    min(df$f0Floor4real, na.rm=T) - .1 * df$f0_token_range, 
  ...
  ifelse(
    sum(df$smogPP_12Hz, na.rm=T) < 5, 0, # minimum = 50 (ms) * 0.1 (smogPP) = 5
    round(sum((df$f0Post - f0Floor) * df$t * df$smogPP_12Hz, na.rm=T) / sum((df$f0Post - f0Floor) * df$smogPP_12Hz, na.rm=T),3))
  }
#
...
comp_df <- mutate(
  ...
  DeltaF0 = ifelse(
    min(which(!is.na(CoM)), na.rm = T) == t+1, f0atCoM - f0_token_median, 
  ...
  DeltaF0_rel = round((DeltaF0 / f0_token_range) * 100, 1),
  # DeltaF0_rel = round((DeltaF0 / f0_speaker_range) * 100, 1),
  sync_rel = round((synchrony / intervalDuration) * 100, 2),
...
####################### F0 curves
  ...
  ##### periogram (smogPP)
  geom_line(aes(y=f0Post),color="blue", alpha=single_token$smogPP_12Hz, size=single_token$smogPP_12Hz*3.5) +
...
mini_comp_df <- droplevels(subset(comp_df, select = -c(plotFloorToken, plotFloorSpeaker, plotFloorData, intensityRel, strengThresh_indeed, totalPowerRel, postPP_rel, logPP_rel, smogPP_1stDer, smogPP_2ndDer, smogPP_2ndDerDyn, CoM_seq, CoM_diff, CoM_diff_rel, CoM_diff_relInv, CoM_diff_relInterp, CoM_diff_relInterpStretch, CoM_diff_relSmoothInterp)))
